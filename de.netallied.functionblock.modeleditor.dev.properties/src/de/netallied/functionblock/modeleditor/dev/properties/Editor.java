 ...
 */
package de.netallied.functionblock.modeleditor.dev.properties;
...
public class Editor extends EditorPart implements IDeviceEditorPart
{
  ...
  public void createPartControl(Composite parent)
  {
    ...
    nameText.setText(getDeviceTypeModel().getName());
    nameText.addModifyListener(new ModifyListener()
    {
      public void modifyText(ModifyEvent e)
      {
        try
        {
          getDeviceTypeModel().setName(nameText.getText());
          setDirty();
        }
        catch (Exception e1)
        {
    ...
    classificationText.setText(getDeviceTypeModel().getClassification());
    classificationText.addModifyListener(new ModifyListener()
    {
      public void modifyText(ModifyEvent e)
      {
        String c = classificationText.getText();
        if(!getDeviceTypeModel().getClassification().equals(c))
        {
          getDeviceTypeModel().setClassification(c);
          setDirty();
        }
      }
    });
    ...
    commentText.setText(getDeviceTypeModel().getComment());
    commentText.addModifyListener(new ModifyListener()
    {
      public void modifyText(ModifyEvent e)
      {
        String c = commentText.getText();
        if(!getDeviceTypeModel().getComment().equals(c))
        {
          getDeviceTypeModel().setComment(c);
          setDirty();
        }
      }
    });
