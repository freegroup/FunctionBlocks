...
public class InputEventModel extends AbstractEventModel implements AbstractTargetModel
{
  
  public InputEventModel(AbstractFunctionBlockModel parent, String name)
  {
    super(parent, new Event());
    getCastor().setName(name);
  }

  public InputEventModel(AbstractFunctionBlockModel parent, Event event)
	{
    super(parent, event);
	}
  ...
  public List<AbstractValueModel> getWithElements()
  {
    ...
    for (int i = 0; i < getCastor().getWithCount(); i++)
    {
      ...
      result.add(getAbstractFunctionBlockModel().getInputValueModel(with.getVar()));
    }
    return result;
  }
